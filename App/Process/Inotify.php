...
class Inotify extends   AbstractProcess
{
    public function run(Process $process)
    {
        if(extension_loaded('inotify')){
            ...
            Logger::getInstance()->console('文件和目录数量'.count($list));
            $notify = inotify_init();
            // 为所有目录和文件添加inotify监视
            foreach ($list as $item) {
                Logger::getInstance()->console('监视 '.$item);

                $flag = inotify_add_watch($notify, $item, IN_CREATE | IN_DELETE | IN_MODIFY);
                if(!$flag)
                    Logger::getInstance()->console('监视 '.$item.'失败');
...
//            ServerManager::getInstance()->getServer()->reload();
            ...
            swoole_event_add($notify, function () use ($notify) {
                ...
                Logger::getInstance()->console(json_encode($events));
                if (!empty($events)) {
            ...
            Logger::getInstance()->console('auto reload unenable');
        }
    }
